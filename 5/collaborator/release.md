...
それでも解決しない場合、Packagist の依存関係などを管理するメタ情報が更新されていない可能性があります。
その場合、更新されるのを待つしかありません。
...
```
次のバージョンが 5.1.1 の場合
~~以前は、5.1.1-dev というようにサフィックスを付けていましたが 5.1.0 以降からは付けなくなりました。~~

2026/03/08 以降、-dev サフィックスを付けることとなりました。
