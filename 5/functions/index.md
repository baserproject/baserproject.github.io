...
| [パスワードの複雑度設定機能](./baser-core/good-password) | パスワードの複雑度設定機能は、パスワードの複雑度合いを設定する機能です。す。 |
...
BcBurgerEditor プラグインにより提供。
固定ページやブログ記事の本文を、ブロックを積み上げるように組み立てられるエディタです。インストール時に標準のエディタタイプとして設定されます。
詳細については、[BurgerEditor](../operation/application/burger_editor) をご覧ください。

| 機能            | 説明                                                             |
|---------------|----------------------------------------------------------------|
| ブロックの追加       | 見出し・テキスト・画像・ボタン・表・ギャラリーなど、用意されたブロックを選んで本文に挿入できます。              |
| ブロックの編集       | ブロックの種類ごとの専用ダイアログで内容を編集できます。                                    |
| ブロックの並べ替え・削除  | ブロック単位で上下に移動したり、削除したりできます。                                      |
| ブロックのコピー      | ブロックをコピーして、別の固定ページやブログ記事に貼り付けられます。                              |
| オプション設定       | ブロックごとに下余白・背景色・枠線・独自class・ID属性を設定できます。                          |
| 画像・ファイルの登録    | 編集ダイアログから画像やファイルをアップロードできます。画像は表示サイズに合わせて自動的に縮小されます。            |
| 外部サービスの埋め込み   | GoogleマップやYouTubeなどを埋め込めます。                                     |
| 本稿・下書き        | 下書き機能が有効な場合、本稿と下書きを切り替えて編集できます。                                 |
...
BcMcp プラグインにより提供。
ClaudeやChatGPTなどのAIエージェントから、会話でサイトのコンテンツを操作できる機能です。AIエージェントと外部サービスをつなぐ共通仕様である MCP（Model Context Protocol）に沿った窓口を提供します。
詳細については、[AIエージェント連携](../operation/application/ai_agent) をご覧ください。

| 機能               | 説明                                                                                  |
|------------------|-------------------------------------------------------------------------------------|
| MCPサーバー          | AIエージェントの接続先となるMCPサーバーを公開します。HTTPでの接続に対応したMCPクライアントから利用できます。                        |
| 固定ページの操作         | 固定ページの取得・作成・編集・削除が行えます。                                                             |
| ブログの操作           | ブログ記事・ブログ・ブログカテゴリ・ブログタグの取得・作成・編集・削除が行えます。                                          |
| カスタムコンテンツの操作     | カスタムコンテンツ・カスタムエントリー・カスタムフィールド・カスタムテーブル・カスタムリンクの取得・作成・編集・削除が行えます。                   |
| 接続の認可            | AIエージェントからの接続について、ユーザーが管理画面で明示的に許可する仕組みを提供します。                                     |
| 権限による制御          | AIエージェントは連携を許可したユーザーの権限で動作します。ユーザーグループの認証プレフィックス設定とアクセスルールにより、操作できる範囲を制限できます。      |
| MCPサーバー管理        | 接続用URL、対応プロトコルバージョン、利用可能なツール、直近の接続状況を確認できます。                                       |
