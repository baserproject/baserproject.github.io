...
![BurgerEditor の編集画面](../../img/burger_editor_index.png)

## エディターを切り替える

BurgerEditor は baserCMS に標準搭載されているコアプラグインです。 **baserCMS をインストールすると自動的に有効化され、エディタタイプも BurgerEditor に設定されます** 。そのため、インストール直後からそのまま利用できます。

エディターを切り替えたい場合は、管理画面の「システム管理」>「システム基本設定」を開き、 **エディタタイプ** から選択して保存してください。

なお、エディターを切り替えても **すでに保存済みの本文はそのまま残ります** 。ただし BurgerEditor で作成した本文を通常のエディターで開くと、ブロックの構造がそのまま HTML として表示されるため、編集には注意が必要です。

## ブロックの操作メニュー

本文のブロックにマウスを乗せると、そのブロックの操作メニューが表示されます。ブロックに対する操作は、すべてこのメニューから行います。

![ブロックの操作メニュー](../../img/burger_editor_block_menu.png)

アイコンは、ブロックの上・左・右に分かれて表示されます。
...
操作メニューの **上にブロックを追加**（＋の上向きの山）または **下にブロックを追加**（＋の下向きの山）をクリックすると「要素を追加」ダイアログが開き、追加できるブロックの一覧が表示されます。使いたいブロックをクリックすると、その位置にブロックが挿入されます。

![要素を追加ダイアログ](../../img/burger_editor_add_block.png)
...
操作メニューの **編集**（鉛筆）をクリックすると、そのブロック専用の編集ダイアログが開きます。

入力内容はブロックの種類ごとに異なります。編集が終わったら **完了** をクリックすると、内容が本文に反映されます。 **キャンセル** をクリックした場合、編集内容は破棄されます。

![画像ブロックの編集ダイアログ](../../img/burger_editor_edit_image.png)
...
操作メニューの **ひとつ上へ移動**（↑）と **ひとつ下へ移動**（↓）で、ブロックの順番を入れ替えられます。

**ブロックを削除**（×）をクリックすると、そのブロックを削除します。 **削除したブロックは元に戻せません。** 確認のメッセージが表示されるので、内容をよく確認してから実行してください。

## ブロックをコピーして使い回す

同じ構成のブロックを繰り返し使いたい場合は、コピー機能が便利です。

1. コピーしたいブロックの **ブロックをコピー**（重なった四角）をクリックします
...
操作メニューの **オプション設定**（歯車）をクリックすると、そのブロックの見た目を調整できます。

![オプション設定ダイアログ](../../img/burger_editor_options.png)
