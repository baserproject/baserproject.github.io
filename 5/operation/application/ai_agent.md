...
管理画面の「システム」→「プラグイン管理」から「BcMcp」をインストールしてください。プラグインの操作方法は「[プラグイン管理](./plugin)」を参照してください。
...
## 接続用URLの確認
管理画面の「システム」→「MCPサーバー管理」を開きます。

<!-- TODO: スクリーンショットを 5/img/ai_agent_index.png として追加したうえで、次の行のコメントを解除してください
...
1. 管理画面の「システム」→「ユーザー管理」→「ユーザーグループ」を開きます
2. 対象のグループを編集し、「Admin Web API」を有効にします
3. アクセスルールから、操作を許可する範囲を調整します

アクセスルールの設定方法は「[アクセスルール管理](./access_rule)」を参照してください。
